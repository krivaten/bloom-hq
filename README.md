...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create bloom-hq
  fly apps create bloom-hq-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app bloom-hq
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app bloom-hq-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'bloom-hq-staging' does not match app name in config file 'bloom-hq'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name bloom-hq-db
  fly postgres attach --app bloom-hq bloom-hq-db

  fly postgres create --name bloom-hq-staging-db
  fly postgres attach --app bloom-hq-staging bloom-hq-staging-db
  ```
